	...
	public static void execute(LevelAccessor world, double x, double y, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("tagName") == 1 && ((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) instanceof Player _plr ? _plr.experienceLevel : 0) >= 1) {
			...
			if (world instanceof ServerLevel _level) {
				...
				entityToSpawn.moveTo(Vec3.atBottomCenterOf(new BlockPos(x, y, (entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getZ() + 2)));
				entityToSpawn.setVisualOnly(false);
				_level.addFreshEntity(entityToSpawn);
			}
			if (world instanceof ServerLevel _level) {
				LightningBolt entityToSpawn = EntityType.LIGHTNING_BOLT.create(_level);
				entityToSpawn.moveTo(Vec3.atBottomCenterOf(new BlockPos(x, y, (entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getZ() + 3)));
				entityToSpawn.setVisualOnly(false);
				_level.addFreshEntity(entityToSpawn);
			}
			if (world instanceof ServerLevel _level) {
				LightningBolt entityToSpawn = EntityType.LIGHTNING_BOLT.create(_level);
				entityToSpawn.moveTo(Vec3.atBottomCenterOf(new BlockPos(x, y, (entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null).getZ() + 4)));
				entityToSpawn.setVisualOnly(false);
				_level.addFreshEntity(entityToSpawn);
			}
		}
